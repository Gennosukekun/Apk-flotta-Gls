...
class PhotoActivity : AppCompatActivity() {
    ...
    override fun onCreate(b: Bundle?) {
        ...
        take.setOnClickListener { openCamera() }; upload.setOnClickListener { uploadAll() }
    }

    private fun openCamera(){if(photos.size>=6)return;val dir=File(cacheDir,"vehicle_photos").apply{mkdirs()};val file=File.createTempFile("mezzo_${System.currentTimeMillis()}_",".jpg",dir);pendingFile=file;val uri:Uri=FileProvider.getUriForFile(this,"$packageName.fileprovider",file);camera.launch(uri)}
    ...
    private fun uploadAll(){if(photos.isEmpty())return;upload.isEnabled=false;take.isEnabled=false;show("Caricamento 0/${photos.size}…");uploadOne(0)}
    private fun uploadOne(index:Int){if(index>=photos.size){show("${photos.size} foto salvate");upload.isEnabled=true;take.isEnabled=photos.size<6;return};val token=intent.getStringExtra("device_token").orEmpty().ifBlank{getSharedPreferences("device",MODE_PRIVATE).getString("device_token","").orEmpty()};if(token.isBlank()){show("Dispositivo non associato: autenticazione mancante");upload.isEnabled=true;take.isEnabled=photos.size<6;return};val bytes=try{jpeg(photos[index])}catch(e:Exception){show("Errore lettura foto ${index+1}");upload.isEnabled=true;take.isEnabled=photos.size<6;return};val builder=MultipartBody.Builder().setType(MultipartBody.FORM).addFormDataPart("assignment_id",intent.getStringExtra("assignment_id").orEmpty()).addFormDataPart("driver_code",intent.getIntExtra("driver_code",0).toString()).addFormDataPart("note",findViewById<EditText>(R.id.note).text.toString());if(initialHabitual){builder.addFormDataPart("photo_type","abituale_iniziale").addFormDataPart("plate",plate)};val body=builder.addFormDataPart("photo","mezzo_${index+1}.jpg",bytes.toRequestBody("image/jpeg".toMediaType())).build();val request=Request.Builder().url(Api.BASE+"/vehicle-photo").header("Authorization","Bearer $token").post(body).build();Api.client.newCall(request).enqueue(object:Callback{override fun onFailure(c:Call,e:IOException){show("Errore rete sulla foto ${index+1}");runOnUiThread{upload.isEnabled=true;take.isEnabled=photos.size<6}};override fun onResponse(c:Call,r:Response){r.use{if(r.isSuccessful){show("Caricamento ${index+1}/${photos.size}…");uploadOne(index+1)}else{show("Errore caricamento foto ${index+1}: ${r.code}");runOnUiThread{upload.isEnabled=true;take.isEnabled=photos.size<6}}}}})}
