...
class DamageReportActivity:AppCompatActivity(){
 ...
 override fun onCreate(b:Bundle?){super.onCreate(b);val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(20),dp(52),dp(20),dp(28))};setContentView(ScrollView(this).apply{setBackgroundColor(getColor(R.color.app_background));addView(root)});fun tv(s:String,z:Float=16f,bold:Boolean=false)=TextView(this).apply{text=s;textSize=z;setTextColor(getColor(R.color.text_primary));if(bold)setTypeface(typeface,Typeface.BOLD);setPadding(0,dp(5),0,dp(5))};root.addView(tv("SEGNALAZIONE MEZZO",12f,true));root.addView(tv("Danno / anomalia",30f,true));root.addView(tv("Furgone assegnato: $plate",17f,true));val categories=arrayOf("Carrozzeria","Pneumatici","Luci","Spie/guasto","Interni","Pulizia","Altro");val spinner=Spinner(this).apply{adapter=ArrayAdapter(this@DamageReportActivity,android.R.layout.simple_spinner_dropdown_item,categories);background=shape(getColor(R.color.surface_input),14,getColor(R.color.border_input));layoutParams=LinearLayout.LayoutParams(-1,dp(54)).apply{topMargin=dp(18)}};root.addView(spinner);val desc=EditText(this).apply{hint="Descrivi il danno o l'anomalia";gravity=android.view.Gravity.TOP;minLines=5;setTextColor(getColor(R.color.text_primary));setHintTextColor(getColor(R.color.text_secondary));background=shape(getColor(R.color.surface_input),14,getColor(R.color.border_input));setPadding(dp(14),dp(14),dp(14),dp(14));layoutParams=LinearLayout.LayoutParams(-1,-2).apply{topMargin=dp(12)}};root.addView(desc);preview=ImageView(this).apply{adjustViewBounds=true;layoutParams=LinearLayout.LayoutParams(-1,dp(190)).apply{topMargin=dp(12)}};root.addView(preview);photoInfo=tv("Foto allegate: 0/6",14f,true);root.addView(photoInfo);take=Button(this).apply{text="Scatta foto (facoltativa)";isAllCaps=false;layoutParams=LinearLayout.LayoutParams(-1,dp(54)).apply{topMargin=dp(8)}};root.addView(take);send=Button(this).apply{text="Invia segnalazione";isAllCaps=false;setTextColor(Color.WHITE);setTypeface(typeface,Typeface.BOLD);backgroundTintList=android.content.res.ColorStateList.valueOf(getColor(R.color.primary));layoutParams=LinearLayout.LayoutParams(-1,dp(56)).apply{topMargin=dp(12)}};root.addView(send);msg=tv("",14f,true);root.addView(msg);take.setOnClickListener{requestCamera()};send.setOnClickListener{val text=desc.text.toString().trim();if(text.isBlank()){msg.text="Inserisci una descrizione";return@setOnClickListener};send.isEnabled=false;take.isEnabled=false;Api.postJson("/damage-reports",JSONObject().put("category",spinner.selectedItem.toString()).put("description",text),token){code,r->if(code in 200..299){reportId=JSONObject(r).optString("id");if(photos.isEmpty())done()else upload(0)}else runOnUiThread{msg.text=runCatching{JSONObject(r).optString("error")}.getOrNull()?.takeIf{it.isNotBlank()}?:"Errore invio ($code)";send.isEnabled=true;take.isEnabled=photos.size<6}}}}
